#!/bin/bash
# Multi-turn rollout sweep across 5 model tiers + 6 seeds × 5 samples × 15 turn pairs.
set -u
cd /root/persona_coinflip
mkdir -p results/rollouts
LOG=results/rollouts/sweep.log
echo "=== rollout sweep started $(date) ===" | tee -a $LOG
...
# 5 tiers — same as EXP-4
run_one "llama-3.1-8b-instruct" "meta-llama/Llama-3.1-8B-Instruct"

run_one "em-llama-8b-sports" \
...
echo "=== rollout sweep finished $(date) ===" | tee -a $LOG
